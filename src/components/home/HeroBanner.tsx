...
import { HERO_BANNER_DEFAULT_VIEW, type HeroBannerView } from "@/lib/hero-banner";

const POSITION_CLASS: Record<HeroBannerView["heroImagePosition"], string> = {
...
export function HeroBanner() {
    const [d, setD] = useState<HeroBannerView>(HERO_BANNER_DEFAULT_VIEW);

    useEffect(() => {
        fetch("/api/hero-banner")
            .then(r => r.json())
            .then(res => { if (res.success) setD({ ...HERO_BANNER_DEFAULT_VIEW, ...res.data }); })
            .catch(() => {});
    }, []);
            ...
            <div className="flex flex-col lg:flex-row min-h-[420px]">
                ...
                <div className="relative w-full lg:w-1/2 min-h-[300px] lg:min-h-[420px]">
                    <Image
                        ...
                        className={`object-cover ${POSITION_CLASS[d.heroImagePosition]}`}
                        priority
                        unoptimized={d.hasCustomHeroImage}
                    />
                </div>
                        ...
                        <div className="flex flex-wrap gap-4 mb-6">
                            ...
                            <div className="flex items-center gap-3 bg-white px-5 py-4 rounded-3xl border border-gray-100 shadow-sm flex-1 min-w-[190px]">
                                <div className="w-10 h-10 flex items-center justify-center bg-gray-50 rounded-xl shrink-0">
                                    {d.badge2IconUrl ? (
                                        <Image src={d.badge2IconUrl} alt={d.badge2IconAlt} width={24} height={24} className="h-6 w-6 object-contain" unoptimized />
                                    ) : (
                                        <svg viewBox="0 0 814 1000" className="w-5 h-5 fill-current text-gray-800" aria-label={d.badge2IconAlt} role="img">
